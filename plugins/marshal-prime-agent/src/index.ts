// marshal-prime-agent — connect a Prime Agent conversation to the marshal coordination
// daemon so it can see and message sibling agent sessions (Claude Code shims,
// opencode sessions, Codex hooks, other Prime Agent/pi sessions, and the TUI)
// on the same roster.
//
// This is the Prime Agent-native counterpart of the Claude Code `marshal-shim`
// and the opencode `marshal-opencode` plugin. Where Claude needs a shim binary (MCP
// child), a proprietary `notifications/claude/channel` push, and ~/.claude
// session-id discovery, Prime Agent's inherited pi extension API gives us all
// of it in-process:
//
//   • tools → native Prime Agent tools (marshal_*); the acting session id is filled
...
// MARSHAL_OPERATOR (optional) in the environment Prime Agent runs under.
...
async function init(pi: ExtensionAPI) {
  ...
  const log = (msg: string) => console.log(`[marshal-prime-agent] ${msg}`);
  ...
  function onInboundMessage(meta: NotifyChannelMeta): void {
    ...
    void (async () => {
      try {
        if (text) {
          // A successful daemon live push has already marked the durable
          // message read. Do not drain here: that can consume a different
          // unread message before it has been injected.
          const content = meta.body_truncated
            ? `${text}\n[preview truncated; read the full message with marshal_messages${meta.message_id ? ` (message ${meta.message_id})` : ""}]`
            : text;
          await pi.sendMessage(
            { customType: "marshal-channel", content, display: true, details: meta },
