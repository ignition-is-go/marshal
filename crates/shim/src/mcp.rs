    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
                ...
                let line = match &msg {
                    OutboundMessage::Reply { id, result } => {
                        ...
                        match serde_json::to_string(&r) {
                            Ok(s) => s,
                            Err(e) => {
                                tracing::warn!(error = ?e, "serialize reply");
                                continue;
                            }
                        }
                    }
                    OutboundMessage::Error { id, body } => {
                        ...
                        match serde_json::to_string(&r) {
                            Ok(s) => s,
                            Err(e) => {
                                tracing::warn!(error = ?e, "serialize error");
                                continue;
                            }
                        }
                    }
                    OutboundMessage::Notification { method, params } => {
                        ...
                        match serde_json::to_string(&n) {
                            Ok(s) => s,
                            Err(e) => {
                                tracing::warn!(error = ?e, "serialize notification");
                                continue;
                            }
                        }
                    }
    ...
    loop {
        ...
        let req: JsonRpcRequest = match serde_json::from_str(trimmed) {
            Ok(r) => r,
            Err(e) => {
                tracing::warn!(error = %e, line = %trimmed, "malformed json-rpc");
